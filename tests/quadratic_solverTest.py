...
TIMEOUT = 15  # seconds
ENCODING = 'UTF8'
TARGET_FILE = 'Test_Code/quadratic_solver.py'

with open("testvalues.json") as fh:
    tests = json.load(fh)["QUADRATIC_SOLVER_TESTS"]
...
class TestQuadratic(unittest.TestCase):
    ...
    def test_unit(self):
        ...
        num = 3
        try:
            import Test_Code.quadratic_solver as qs
        except ImportError:
            return
        for i in range(num):
            expected = UNIT_TESTS[i][3]
            actual = qs.calc_discriminant(UNIT_TESTS[i][0],UNIT_TESTS[i][1],UNIT_TESTS[i][2])
